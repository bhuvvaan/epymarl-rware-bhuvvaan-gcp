...
unique_grid_count = 0
max_iterations = 50  # Set a limit for iterations to avoid infinite loops
...
if len(flattened_grid_set) >= 5:
    ...
    for index, flattened_grid in enumerate(random_selected_grids):
        ...
        # Flatten the grid
        flattened_grid = [cell for row in grid for cell in row]
        # Replace all '@' with 'x' in the flattened grid
        flattened_grid = ['x' if cell == '@' else cell for cell in flattened_grid]
        # Join the flattened grid as a string
        layout_str = ''.join(flattened_grid)
        print(f"Layout String {index + 1}:")
        print(layout_str)
